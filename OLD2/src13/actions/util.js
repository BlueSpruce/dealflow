...
export const getStatusObj = x => getObj(x.ProjectStatus_Id,projStatuses)

const strName = x => R.prop('Name', x )

export const data = R.map(x => R.merge(x, {projectType: strName(getTypesObj(x,projTypes))} ), projects)

const data2 = R.map(x =>
                    R.merge(x,{statusType:strName(getStatusObj(x))}),
                    data)
