...
const notifications = (state = [], action) => {
  ...
  switch (action.type) {
    case "ACTS2":
      console.log("reducer ACTS2 action.name and value and Id " + [action.name, action.value, action.Id]);
    ...
    let v2 = state.projects.map(x => (x.Id === action.Id ? { ...x,
      [action.name]: action.value ? action.value[0].trim(): action.value } : x));
