...
/** FOR TEXT FIELDS **/
    const doTextField = async (field) => {
      ...
      dispatch(addSelectedField2(curProjId,field, statusNotes));
      pushToAllListItems(resultField, field);
    }
    const selectFields =[{name:"familymembers"},{name:"leadPerson"},
  {name:"industry"}, {name:"industrysubtype"}, {name:'financials'},
{name:'legal'}, {name:'background'}, {name:'reviewStatus'}]
   /* dispatch select fields */
    R.map(x=>doField(x.name),selectFields)
    ...
    doTextField("statusNotes")
    /*Key people */
    doTextField("keypeople")
    /* recommendation */
    doTextField("recommendation")
