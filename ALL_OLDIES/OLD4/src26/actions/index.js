...
export const selector = n => async (dispatch, getState) => {
  ...
  if (getState().data.projects) {
    ...
    const doCurrencyField =  async field => {
      ...
      const resultValue =
        customFieldValue[`CustomField${resultField.FieldNumber}`];
        dispatch(addSelectedField2(curProjId, deleteDollarSign(field), resultValue));


    }
    /* MIN CAPITAL */
   doCurrencyField("minCapital")

    /* MAX CAPITAL */
     doCurrencyField("maxCapital")
    /*  CAPITAL committed */

 doCurrencyField("capitalCommitted")
 
    const textFields = [{name:'statusNotes'},{name:'keypeople'},{name:'recommendation'}]
    /* dispatch select fields */
